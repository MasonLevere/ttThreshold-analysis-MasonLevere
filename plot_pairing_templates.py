#!/usr/bin/env python3
"""
For matched events (dR < 0.1), compare the mass distribution of each method's
chosen permutation against the true (gen-level correct) permutation.
Runs over all 4 pairing methods across ALL matched events (not just wrong ones).
"""
import numpy as np
import uproot
import matplotlib
...
MATCH_DR = 0.1
BASE_DIR = "plots/pairing_templates"
...
for pfx, label in METHODS:
    ...
    n_correct = (C[f"{pfx}_pairing"][m] == true[m]).sum()
    print(f"\n── {label} ──")
    print(f"  matched events : {m.sum()}  correct: {n_correct} ({100*n_correct/m.sum():.1f}%)")

    SUPTITLE = (f"{label} — true vs chosen perm  "
                f"(all matched dR<{MATCH_DR}, n={m.sum()}, eff={n_correct/m.sum():.3f})")
    ...
    im0 = axes[0].imshow(H_true.T,   origin="lower", extent=ext, aspect="auto", cmap="Blues")
    axes[0].set_title("true permutation  p(ma, mb | true)")
    plt.colorbar(im0, ax=axes[0], label="density")

    im1 = axes[1].imshow(H_chosen.T, origin="lower", extent=ext, aspect="auto", cmap="Oranges")
    axes[1].set_title(f"{label} chosen perm  p(ma, mb | chosen)")
    ...
    plt.tight_layout()
    out = os.path.join(outdir, "templates_2d.png")
    ...
    axes[1].hist(chosen_ma + chosen_mb, bins=bins_sum, density=True, histtype="step",
                 lw=2, color="C1", label="chosen perm", ls="--")
    ...
    plt.suptitle(SUPTITLE, y=1.01)
    plt.tight_layout()
    out = os.path.join(outdir, "templates_1d.png")
    plt.savefig(out, dpi=120, bbox_inches="tight"); plt.close(); print("wrote:", out)
    ...
    eff   = (np.argmax(score, 1) == t_m).mean()
    print(f"  template recovery (inflated, all matched): {eff:.4f}")
